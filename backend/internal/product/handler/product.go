...
func (h *ProductHandler) GetLowStock(c *fiber.Ctx) error {
	...
	resp, err := h.service.GetLowStockProducts(userID, page, limit)
	if err != nil {
		if err.Error() == "unauthorized" {
			return c.Status(fiber.StatusForbidden).JSON(fiber.Map{"error": "Access denied. Seller profile required."})
		}
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"error": "Failed to get low stock products: " + err.Error()})
	}
