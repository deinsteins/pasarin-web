...
export function SocialButton({ provider, children, className = "", ...props }: SocialButtonProps) {
  ...
  return (
    <button
      type="button"
      className={`flex w-full items-center justify-center gap-2.5 rounded-soft-md border border-zinc-200 bg-white py-3 px-4 text-sm font-semibold text-zinc-700 shadow-soft-sm transition-all duration-200 hover:bg-zinc-50 hover:shadow-soft-md active:scale-[0.98] cursor-pointer dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-300 dark:hover:bg-zinc-850 ${className}`}
      {...props}
    >
      {icons[provider]}
      <span>{children}</span>
